...
function ResetPassword() {
  ...
  const handleSubmitBtnClick = async () => {
    ...
    try {
      ...
      const response = await requestHandler("/users/reset-password", "POST", params);
            ...
            <>
              {!isSuccess ? (
                ...
                <div className="w-full max-w-lg rounded-2xl bg-white shadow-lg border border-gray-200 p-10 text-center">
                  ...
                  <h1 className="font18 font-semibold color-3">Password Reset Successful</h1>
                  <p style={{ marginBottom: 0, marginInline: 0 }} className="margin-34 color-4 line-h-8 font-9">
                    Your password has been successfully reset.
                    <br />
                    For your security, you've been signed out on all devices. Please sign in using your new password.
                  </p>
                  <button onClick={navigateToHome} className="mt-8 w-full rounded-lg bg-[#000000] px-5 py-3 text-white font-medium cursor-pointer">
                    Continue to Sign In
                  </button>
                </div>
              )}
