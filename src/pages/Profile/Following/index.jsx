...
function Following() {
  ...
  const fetchPeopleFollowingList = async (skip) => {
    try {
      ...
      if (loaders.peopleLoader) setLoaders((prev) => ({ ...prev, peopleLoader: false }));
    } catch (err) {
      console.error(err);
      if (loaders.peopleLoader) setLoaders((prev) => ({ ...prev, peopleLoader: false }));
    }
  ...
  useEffect(() => {
    if (user && loaders.peopleLoader) fetchPeopleFollowingList(0);
  }, [user]);
      ...
      <div className="margin-12 min-w-0 w-full max-width-2">
        ...
        <nav className="padding70 bdr-5" style={{ paddingBottom: 0, paddingInline: 0, borderTop: 0, borderInline: 0 }}>
          <div className={`inline-block custom-px-2 margin-18 ${activeTabIndex === 0 ? "bdr-7" : ""}`} style={{ borderTop: 0, borderInline: 0, marginLeft: 0 }}>
            <button onClick={() => handleActiveTabChange(0)} disabled={defaultLoader || loaders.peopleLoader} className="cursor-pointer p-0 m-0">
          ...
          <div className={`inline-block custom-px-2 ${activeTabIndex === 1 ? "bdr-7" : ""}`} style={{ borderTop: 0, borderInline: 0 }}>
            <button onClick={() => handleActiveTabChange(1)} disabled={defaultLoader || loaders.peopleLoader} className="cursor-pointer p-0 m-0">
