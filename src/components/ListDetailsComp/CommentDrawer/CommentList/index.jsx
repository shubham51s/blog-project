...
function CommentList({ item, setList }) {
  ...
  const handleUpdateComment = async () => {
    ...
    try {
      ...
      const response = await requestHandler("/list/comment/update", "POST", params);
      const result = await response.json();
      ...
      if (response?.status === 200 && result?.data?.content) {
        ...
        response?.status < 500 ? showToast(result?.message || "Some error occured.") : showToast("Some error occured.");
      }
    } catch (err) {
      console.error(err);
      showToast("Some error occured.");
                    ...
                    <div className="padding-33" style={{ paddingRight: 0, paddingBlock: 0 }}>
                      <div className="flex items-center">
                        <div className="cursor-pointer transition-all duration-400 ease-in-out hover:underline">
                          <p className="break-all text-ellipsis color-3 custom-fs-1 overflow-hidden font-normal m-0 p-0">{comment.user.name}</p>
                        </div>
                            ...
                            {comment.user._id === userInfo._id && (
                              <ul className="padding-6 flex flex-col items-stretch list-none m-0" style={{ paddingInline: 0 }}>
                                <li className="padding-1 custom-fs-1 color-4 font-normal">
                                  <button onClick={handleEditBtnClick} className="color-3 cursor-pointer m-0 p-0 opacity-[0.9] transition-all duration-75 ease-in hover:opacity-100" disabled={isLoading}>
                                    Edit response
                                  </button>
                                </li>
                                <li className="padding-1 custom-fs-1 color-4 font-normal">
                                  <button onClick={() => handleDeleteComment(comment._id)} className={`text-[#b63636] cursor-pointer m-0 p-0 transition-all duration-75 ease-in ${isLoading ? "opacity-[0.5]" : "opacity-[0.9] hover:opacity-100"}`} disabled={isLoading}>
                                    Delete response
                                  </button>
                                </li>
                              </ul>
