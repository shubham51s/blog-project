...
function Blogs({ publication }) {
  ...
  const getPublicationBlogs = async () => {
    ...
    try {
      const url = scroll.current.cursor ? `/blogs/publication/${publication._id}?sort=${selected.current.value}&cursor=${scroll.current.cursor}&limit=${limit}` : `/blogs/publication/${publication._id}?sort=${selected.current.value}&limit=${limit}`;
