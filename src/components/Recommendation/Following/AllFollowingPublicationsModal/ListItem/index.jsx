...
function ListItem({ item, onPublicationFollowStatusChange }) {
          ...
          <div className="padding82 w-full flex justify-between">
            <div className="w-full flex flex-col justify-center">
              <div className="flex items-center">
                <Link to={`/profile/${item.username}`} className="cursor-pointer m-0 no-underline p-0">
              ...
              {item.bio && (
                <Link to={`/profile/${item.username}`} className="cursor-pointer m-0 no-underline p-0">
                  <div className="w-full max-w-full whitespace-pre-wrap margin44 break-words">
                    <p className="custom-fs-1 color-4 line20 font-normal m-0">{item.bio}</p>
                  </div>
                </Link>
              )}
            </div>
