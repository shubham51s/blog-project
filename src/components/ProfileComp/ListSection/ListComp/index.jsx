...
function ListComp({ user, setUser, item, filterOutDeletedList }) {
  ...
  const handleNavigateToLink = () => {
    navigate(`/profile/${user.username}/list/${list.slug}`);
  };
  ...
  return (
    <div className="relative w-full width55 z-0 flex justify-between margin57 bdr-5 border-radius-3 bg-10 cursor-pointer">
      <div className="grow shrink-0 basis-0 padding-3 padding76 flex flex-col break-words justify-between">
        ...
        <Link to={`/profile/${user.username}/list/${list.slug}`} className="block margin-7" style={{ marginBottom: 0, marginInline: 0 }}>
        ...
        <div className="flex items-center justify-between">
          <Link to={`/profile/${user.username}/list/${list.slug}`} className="flex margin-6 items-center">
      ...
      <Link to={`/profile/${user.username}/list/${list.slug}`} className="block">
