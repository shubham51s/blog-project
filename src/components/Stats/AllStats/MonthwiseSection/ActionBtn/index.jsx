...
function ActionBtn({ isLoading, months, selected, handleMonthChange }) {
      ...
      <Popover.Root open={isOpen} onOpenChange={setIsOpen}>
        ...
        <Popover.Portal>
          <Popover.Content onClick={(e) => e.stopPropagation()} side="bottom" align="middle" sideOffset={8}>
            <ul className="list-none border-radius-3 boxShadow6 custom-bg-8 custom-px-2 width107 height82 overflow-x-hidden overflow-y-auto">
              {months.map((item) => (
                <li onClick={() => handleMonthSelection(item.month, item.year)} key={`${item.month}${item.year}`} className={`flex items-center justify-between flex-wrap color-3 cursor-pointer padding-7 transition-all duration-75 ease opacity-[0.75] hover:opacity-100`}>
                  <div className="padding50" style={{ paddingLeft: 0 }}>
                    <span className="color-3 custom-fs-1 line20 font-medium">
                  ...
                  <div className="shrink-0">
                    <div className="padding-6" style={{ paddingRight: 0 }}>
                      <div className={`width84 aspect-square`}>{selected.month === item.month && selected.year === item.year && <IoMdCheckmark className="w-full h-full" />}</div>
                    </div>
                  </div>
                </li>
              ))}
